...
using Auth.Service.Application.Settings.Exceptions;
using Auth.Service.Domain.Exceptions;
using Auth.Service.Domain.Resources;

namespace Auth.Service.Api.Filters;
...
public class CustomExceptionFilter : IExceptionFilter
{
    ...
    public void OnException(ExceptionContext context)
    {
        ...
        MountUserNotFound(context);

        MountUserAlreadyExists(context);
    ...
    private void MountUserNotFound(ExceptionContext context)
    {
        if (context.Exception is not UserEmailNotRegistered)
            return;

        isInternal = false;

        var unfEx = context.Exception as UserEmailNotRegistered;

        var message = unfEx.Message;

        if (unfEx.IsDefaultMessage)
            message = _messages.GetMessage(Messages.Exception.IF_USER_EXISTS_SEND_EMAIL);

        var objeto = new ResponseError(message);

        MountResponse(context, (int)HttpStatusCode.OK, objeto);
    }

    private void MountUserAlreadyExists(ExceptionContext context)
    {
        if (context.Exception is UserAlreadyExistsException uaeEx)
        {
            isInternal = false; 

            var message = uaeEx.Message;

            if (uaeEx.IsDefaultMessage)
            {
                message = _messages.GetMessage(Messages.Exception.USER_ALREADY_EXISTS);
            }

            var objeto = new ResponseError(message);
            MountResponse(context, (int)HttpStatusCode.Conflict, objeto);  // Changed from OK to Conflict
        }
    }
